...
const FAQ_ITEMS = [
 ...
 a: 'Yes. Your TrustOffice subscription is month-to-month. You can cancel from your dashboard at any time. Your trust remains legally valid regardless of subscription status.',
 },
 {
 ...
 a: 'WingPoint and TrustOffice are separate services that work together. WingPoint creates your trust. TrustOffice manages it. Your purchase on WingPoint included a $50 credit toward your TrustOffice management plan.',
 },
];
...
export default function WingPointWelcomePage() {
 ...
 <CheckCircle className="w-5 h-5 text-gold flex-shrink-0 mt-0.5" />
 <p className="text-sm text-muted-foreground">
 <span className="font-medium text-navy">Your $50 credit</span> covers part of your first month.
 </p>
 </div>
 </div>
 </div>
 ...
 <p className="text-2xl font-serif font-bold text-navy mb-1">{pkg.price}</p>
 <p className="text-sm text-muted-foreground mb-4">{pkg.credits}</p>
 <div className="px-3 py-2 bg-gold/10 border border-gold/30 mb-4">
...
function LoggedInAction({ subscription, navigate, plan }) {
 ...
 onClick={() => navigate(`/pricing?wp=1&coupon=WINGPOINT50&plan=${planParam}`)}
 buttonVariant="default"
 />
