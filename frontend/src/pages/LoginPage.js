...
export default function LoginPage() {
  ...
  // 2FA second-step state: after a password-only login returns
  // 401 {detail:'2fa_required', challenge_token}, collect the 6-digit code
  // (or a recovery code) and POST /auth/2fa/login.
  ...
  // Shared post-login success path used by BOTH the normal login and the
  // 2FA second step, so a 2FA login proceeds exactly like a normal one.
  ...
  // 2FA second step: exchange the challenge token + 6-digit code (or a
  // recovery code) for a normal session, then proceed exactly as a normal
  // login would.
          ...
          <div className="card-trust corner-mark relative">
            {twoFactorStage ? (
              <>
                ...
                <form onSubmit={handleTwoFactorSubmit} data-testid="twofa-login-step">
                  <div className="space-y-4">
                    {twoFactorError && (
                      <div className="p-3 bg-error/10 border border-error/20 flex items-start gap-2" data-testid="twofa-login-error">
                        <AlertCircle className="w-4 h-4 text-error flex-shrink-0 mt-0.5" />
                        <div className="flex-1">
                          <p className="text-sm font-medium text-error">Verification failed</p>
                          <p className="text-xs text-error/80 mt-0.5">{twoFactorError}</p>
                        </div>
                      </div>
                    )}
