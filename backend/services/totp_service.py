...
Owns everything 2FA that is not route wiring:
  ...
  - recovery-code generation + bcrypt hashing (same scheme as passwords —
    NOT a fast hash; council review delta 2026-09-12)
...
async def consume_login_code(user_doc: dict, code: str) -> Tuple[bool, str]:
    ...
    record_totp_failure(user_doc.get("user_id", ""))
