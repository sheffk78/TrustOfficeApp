...
# External partner API (auth via API key, not user session)
    ...
    # Public marketing-activity logging (video watch / PDF download by email)
...
app.include_router(state_deep_knowledge_router, prefix="/api")
...
async def startup_event():
    ...
    try:
        ...
        await db.trusts.create_index(
            "wingpoint_ref",
            unique=True,
            partialFilterExpression={"wingpoint_ref": {"$type": "string"}}
        )
