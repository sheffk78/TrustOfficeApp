...
def _login_frequency_score(days_since_login: Optional[int]) -> int:
    """Score login frequency (30 points max)."""
...
def _recency_score(days_since: Optional[int]) -> int:
    """Score capture recency (20 points max — fresh leads score higher)."""
...
def calculate_lead_score(lead: dict) -> int:
    ...
    score += _login_frequency_score(_days_since(lead.get("last_login")))

    # Days since capture (20 points max)
    score += _recency_score(_days_since(lead.get("created_at")))
    ...
    score += SOURCE_QUALITY.get(lead.get("source", ""), 1)
...
def get_score_breakdown(lead: dict) -> dict:
    ...
    login_score = _login_frequency_score(_days_since(lead.get("last_login")))
    recency_score = _recency_score(_days_since(lead.get("created_at")))
...
def _parse_facebook_field_data(field_data: list) -> dict:
    """Extract name, email, phone, city, job_title, situation, looking_for_system from Facebook form data."""
    name = email = phone = city = job_title = situation = looking_for_system = ""
    custom_answers = {}
    ...
    return {
        ...
        "job_title": job_title, "situation": situation,
        "looking_for_system": looking_for_system, "custom_answers": custom_answers,
    }


async def _find_existing_facebook_lead(leadgen_id: str, email: str) -> Optional[dict]:
    """Find an existing lead by leadgen_id (primary) or email (fallback)."""
...
async def _update_facebook_lead(existing: dict, parsed: dict, leadgen_id: str, form_id: str, page_id: str):
    """Update an existing lead with Facebook form data."""
    email = existing.get("email", "")
    now_iso = datetime.now(timezone.utc).isoformat()
    await db.leads.update_one(
        {"email": email},
        {"$set": {
            "name": parsed["name"] or existing.get("name", "Unknown"),
            "source": "facebook-lead-ad",
            "phone": parsed["phone"] or existing.get("phone"),
            "city": parsed["city"] or existing.get("city"),
            "job_title": parsed["job_title"] or existing.get("job_title"),
            "situation": parsed["situation"] or existing.get("situation"),
            "looking_for_system": parsed["looking_for_system"] or existing.get("looking_for_system"),
            "custom_form_answers": parsed["custom_answers"] if parsed["custom_answers"] else existing.get("custom_form_answers", {}),
...
async def _create_facebook_lead(parsed: dict, leadgen_id: str, form_id: str, page_id: str):
    """Create a new lead from Facebook Lead Ad data. Returns the lead_id."""
    name = parsed["name"]
    email = parsed["email"]
    now = datetime.now(timezone.utc)
    lead_id = f"lead_{uuid.uuid4().hex[:12]}"
    ...
    lead_doc = {
        ...
        "looking_for_system": parsed["looking_for_system"],
        "custom_form_answers": parsed["custom_answers"],
        "facebook_leadgen_id": leadgen_id,
        "facebook_form_id": form_id,
        "facebook_page_id": page_id,
        "utm_source": "facebook",
    ...
    # Send Discord notification
    await _send_facebook_discord_notification(parsed, name, email)
    await create_notification(
        type="new_lead", title=f"New Facebook lead: {name}",
        body=f"Source: Facebook Lead Ad · Phone: {parsed['phone'] or 'N/A'}",
...
async def _send_facebook_discord_notification(parsed: dict, name: str, email: str):
    """Send Discord notification for a new Facebook lead."""
    try:
        discord_fields = [
        ...
        if parsed["phone"]:
            discord_fields.append({"name": "Phone", "value": parsed["phone"], "inline": True})
        if parsed["job_title"]:
            discord_fields.append({"name": "Title", "value": parsed["job_title"], "inline": True})
        if parsed["situation"]:
            discord_fields.append({"name": "Situation", "value": parsed["situation"], "inline": False})
        if parsed["looking_for_system"]:
            discord_fields.append({"name": "Looking for system?", "value": parsed["looking_for_system"], "inline": True})
...
async def _process_facebook_entry(entry: dict, results: list):
    ...
    for change in changes:
        ...
        parsed = _parse_facebook_field_data(lead_data.get("field_data", []))
        name = parsed["name"] or lead_data.get("name", "Unknown")
        email = parsed["email"]
        ...
        # Check if lead already exists
        existing = await _find_existing_facebook_lead(leadgen_id, email)
        if existing:
            lead_id = await _update_facebook_lead(existing, parsed, leadgen_id, form_id, page_id)
            results.append({"leadgen_id": leadgen_id, "lead_id": lead_id, "success": True, "is_returning": True})
        else:
            lead_id = await _create_facebook_lead(parsed, leadgen_id, form_id, page_id)
...
async def facebook_webhook_handler(request: Request):
    ...
    for entry in entries:
        await _process_facebook_entry(entry, results)
