...
def _pdf_styles(white_label: bool = False):
    """Build the ParagraphStyle objects used by the minutes PDF.

    When white_label=True, returns de-branded (black text, no TrustOffice brand
    color) styles for advisor-facing documents.
    """
    ...
    title_style = ParagraphStyle(
        ...
        textColor=colors.black if white_label else NAVY,
    )
    ...
    section_header_style = ParagraphStyle(
        ...
        borderColor=colors.black if white_label else NAVY,
    )
...
def _pdf_footer(minutes: dict, trust: dict, hide_watermark: bool, styles, divider_style, trust_name: str,
                white_label: bool = False) -> list:
    """Build the PDF footer (watermark + confidential note)."""
    story = [Spacer(1, 30), Paragraph("═" * 60, divider_style)]
    if not white_label and not hide_watermark:
    ...
    story.append(Paragraph(
        ...
        ParagraphStyle('FooterNote', parent=styles['Normal'], fontName='Times-Italic',
                       fontSize=8, alignment=1, textColor=colors.black if white_label else GRAY),
    ))
    return story
...
async def get_minutes_template_pdf(minutes_id: str, user: dict = Depends(get_current_user)):
    ...
    # Add watermark footer (omitted for white-label advisor exports)
    if show_watermark and not white_label:
