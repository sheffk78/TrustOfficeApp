...
async def export_audit_defense_pdf(trust_id: str, days: int = 365, user: dict = Depends(get_current_user)):
    ...
    if white_label:
        from pdf_utils import build_legal_styles, legal_separator_line
        S = build_legal_styles()
        sep = legal_separator_line
        brand_footer = False
    else:
        S = build_styles()
        sep = separator_line
    ...
    if brand_footer:
        story.append(Paragraph(
            "This report was generated by TrustOffice. "
    ...
    story.append(Paragraph(
        ...
        ParagraphStyle('Disclaimer', parent=S['small'], fontName='Helvetica-Bold',
                       textColor=(NAVY if brand_footer else colors.black))))
