...
Endpoints (API CONTRACT — frontend builds against exactly this):
  POST /auth/2fa/enroll   {password}            -> {secret, provisioning_uri}
       (auth required + password re-auth: a hijacked session cannot enroll
        an attacker's own device — council review delta 2026-09-12)
...
NOTE: this branch is based on the pre-session-hardening main (JWT 7-day
tokens, no refresh-token system). When merged after security/session-hardening,
the two session-response cookie blocks in this file should be updated to the
30-min access + refresh pattern (see merge note in the branch commit).
"""
...
async def login_2fa(request: Request, body: TwoFactorLoginRequest, response: Response):
    ...
    response.set_cookie(
        ...
        samesite="lax", max_age=JWT_EXPIRATION_HOURS * 3600, path="/",
    )
    ...
    if is_admin and not state.get("enabled"):
