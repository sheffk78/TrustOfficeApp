...
def _router_modules():
    ...
    # Rebind the routers' db handle to this module's mongomock db. When
    # another suite (e.g. test_tenant_isolation) imports the routers first,
    # they capture THAT module's db handle; the skeleton tests then seed a
    # different handle and every lookup 404s ("Trust not found"). Idempotent.
    _rm.db = db
    _rmeet.db = db
    return _rm, _rmeet
...
class TestFixPassDARouter:
    ...
    async def test_preparer_can_create_and_update_draft_flag_on(self, seeded_db):
        ...
        os.environ["TOGGLE_INSTITUTION"] = "1"
        # Both flags explicit: require_trust_party_access short-circuits (returns
        # user unchanged) when TOGGLE_TRUST_PARTIES is off, which would skip the
        # org-grant union check — the preparer path must not depend on env leaks.
        os.environ["TOGGLE_TRUST_PARTIES"] = "1"
