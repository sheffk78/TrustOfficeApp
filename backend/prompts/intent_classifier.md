...
| `contribute_asset` | "I'm contributing my house to the trust", "Transfer my car into the trust", "Contribute $50k to the trust corpus", "Put my website into the trust" | Accept/contribute an asset into the trust, generating both Schedule A entry and acceptance resolution minutes |
...
- IMPORTANT: Distinguish between `add_asset` (logging an existing trust asset to Schedule A inventory) vs `contribute_asset` (transferring/contributing a new asset INTO the trust, which also requires acceptance minutes/resolution). "I bought a property" = add_asset. "I'm contributing my house to the trust" / "transfer my car into the trust" / "put my business into the trust" = contribute_asset. "Add my house to the trust" / "add to the trust" with transfer intent = contribute_asset. "Add to Schedule A" / "log an asset" = add_asset.
- IMPORTANT: When the user asks "what do I do for a quarterly review", "what does a quarterly review cover", "help me prepare for a quarterly/annual review", or similar — classify as `check_deadlines` (NOT `ask_knowledge`). This ensures the AI gets full calendar and trust context to give an actionable answer, not a lightweight knowledge-only response.
